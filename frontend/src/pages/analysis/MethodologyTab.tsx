 ...
 *
 * **Расхождения с нормой — первым блоком и отдельно.** В общем списке «открыто к сверке»
 * они читаются как «ещё обсуждается», а у них есть ответ, и он другой.
 */

/**
 * Подпись способа закрытия. Короткая — рядом с самим вопросом; полный текст оговорки
 * («это предложение, а не подтверждение») едет отдельной строкой под расхождениями.
 * Незнакомый ключ подписывается собой, а не пропадает: молча спрятанный способ читался
 * бы как «вопрос закрыт».
 */
const RESOLUTION_LABEL: Record<string, string> = {
  citable: "есть норма — проверить",
  judgement: "нужно суждение",
  presentation: "только представление",
};
...
export function MethodologyTab({ projectId }: { projectId: string }) {
  ...
  // Расхождения идут **первыми и отдельно**: в общем списке «открыто к сверке» они
  // читаются как «ещё обсуждается», а у них есть ответ — и он другой.
  const diverging = data.choices.filter((c) => c.divergence);
  ...
  return (
    <div className="mth">
      ...
      {diverging.length > 0 && (
        <div className="mth-diverge">
          <div className="mth-diverge__title">
            Где расчёт расходится с нормой ({diverging.length})
          </div>
          {diverging.map((c) => (
            <div key={c.id} className="mth-diverge__item">
              <b>{c.number}. {c.title}.</b> {c.divergence}
              {c.proposed_basis && (
                <div className="mth-diverge__basis">Основание: {c.proposed_basis}</div>
              )}
            </div>
          ))}
          <div className="mth-diverge__note">{data.classification_note}</div>
        </div>
      )}
    ...
    <div className={"mth-card" + (c.engaged ? "" : " mth-card--silent")}>
      ...
      {c.open_question && (
        <div className="mth-card__open">
          ...
          {/* Способ закрытия рядом с вопросом, а не в сноске: «нужен аудитор» и
              «есть норма, прочитайте» — разный объём работы для читателя. */}
          <span className={"mth-res mth-res--" + c.resolution}>{RESOLUTION_LABEL[c.resolution] ?? c.resolution}</span>
        </div>
      )}
      {c.proposed_basis && (
        <div className="mth-card__basis">
