...
/**
 * Что стало с письмом со ссылкой (D1) — рядом с самой ссылкой.
 *
 * Три состояния, и они не сводимы одно к другому: **не пытались** (почта в этой
 * установке не настроена), **ушло** и **не ушло с причиной**. Ссылка при этом видна
 * всегда: письмо может не дойти и молча — спам-фильтр, опечатка в адресе, — и
 * передавать её лично остаётся рабочей дорогой, а не запасной.
 */
function MailNote({ mail }: { mail?: { attempted: boolean; ok: boolean; error: string } | null }) {
  if (!mail?.attempted) {
    return <>Писем платформа не отправляет, поэтому передайте ссылку лично.</>;
  }
  if (mail.ok) {
    return <>Письмо со ссылкой отправлено — эта копия на случай, если оно не дойдёт.</>;
  }
  return <><b>Письмо отправить не удалось</b> ({mail.error}) — передайте ссылку лично.</>;
}
