...
describe("balanceGaps — разрыв по периодам", () => {
  ...
  it("пробелы в разрядах понимаются — сервер их тоже принимает", () => {
    // «1 200,50» — обычное русское написание; читать его как ноль значило бы объявить
    // разрыв там, где введено ровно то же число, что и по другую сторону баланса.
    expect(balanceGaps({ A_CASH: ["1 200,50"], P_EQUITY: ["1200.50"] }, 1)).toEqual([0]);
    expect(balanceGaps({ A_CASH: ["1\u00a0200,50"], P_EQUITY: ["1200.50"] }, 1)).toEqual([0]);
  });
});
