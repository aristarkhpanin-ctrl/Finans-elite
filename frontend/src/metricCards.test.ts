...
describe("Потребность в финансировании: два числа — два имени", () => {
  /**
   * Подсказка называла **показанное** число приведённым, а приведено оно не было:
   * в значении стоит наибольший накопленный дефицит в деньгах горизонта
   * (`peak_financing_need` = сумма приростов пика, без дисконтирования), а
   * дисконтированная величина того же графика — в подписи (`pv_investments`).
   *
   * Разница не косметическая: это разные суммы денег, и назвать обе одним именем
   * значит утверждать о деньгах больше, чем посчитано.
   */
  const card = (over = {}) =>
    efficiencyCards(metrics(over), "0.18").find(
      (c) => c.label === "Потребность в финанс.")!;

  it("в значении — недисконтированный пик, и подсказка это говорит", () => {
    const c = card();
    expect(c.hint).toContain("Без дисконтирования");
    expect(c.hint).not.toMatch(/^Приведённая/);
  });

  it("подсказка отсылает к приведённой величине там, где она показана", () => {
    // Иначе читатель решит, что приведённой величины у карточки нет вовсе.
    expect(card().hint).toContain("PV инвестиций");
    expect(card().sub).toContain("PV инвестиций");
  });

  it("без приведённой величины подпись не выдумывает её, а говорит о пике", () => {
    expect(card({ pv_investments: null }).sub).toBe("Максимальный дефицит");
  });
});
