...
class StaffPlanAssign(BaseModel):
    """Назначение тарифа клиенту оператором платформы (F1).
    ...
    #: Чем подтверждена оплата — номер счёта, договор. Свободная строка: платформа не
    #: ведёт бухгалтерию и не делает вид, что проверяет этот номер.
...
class ProfileUpdate(BaseModel):
    """Правка профиля. Каждое поле необязательно, и ``None`` значит «не трогать».
    ...
    #: Письма об обсуждениях (OPEN-DECISIONS §5).
    comment_emails: Optional[bool] = None
...
class EmailVerificationOut(BaseModel):
    """Состояние адреса и что из этого следует.
    ...
    verified: bool
    #: Ушло ли письмо с подтверждением по этому запросу (у проверки состояния — ``None``).
    sent: Optional[bool] = None
    #: Что происходит из-за неподтверждённого адреса — словами, а не намёком.
    note: str = ""
...
class CommentOut(BaseModel):
    ...
    body: str
    mentions: list[str] = []
    #: Ссылки на материалы (OPEN-DECISIONS §6) — **выведены из текста**, а не хранятся
    #: отдельно: удалённая реплика уносит адрес комнаты данных вместе с текстом сама.
    links: list[str] = []
    #: Похожее на адрес, но ссылкой не ставшее: другая схема или путь в локальной сети.
    #: Названо, а не проглочено — иначе автор думает, что приложил выписку.
    unsupported_links: list[str] = []
    #: Что платформа обещает про ссылки. Непусто **только там, где ссылки есть**:
    #: оговорка на реплике без ссылок — шум, из-за которого перестают читать остальные.
    links_note: str = ""
    created_at: datetime
    resolved: bool = False
...
class CommentCreated(BaseModel):
    ...
    #: Кому уйдёт письмо как участнику обсуждения: написал здесь раньше или был упомянут
    #: (OPEN-DECISIONS §5). Автор реплики и позванные по имени сюда не входят — первому
    #: писать незачем, вторым уже написали.
    #:
    #: Это **кого платформа позвала**, а не «кому дошло»: у человека может быть не
    #: подтверждён адрес или выключены письма об обсуждениях, и рассказывать автору
    #: реплики о чужих настройках платформа не станет.
    followed: list[str] = []
    #: Что стало с письмами (общее на всю реплику).
    mail: MailReport = MailReport()


class ThreadSubscriptionOut(BaseModel):
    """Приходят ли этому человеку письма об этой ветке — и что это значит (§5).
...
class ThreadSubscriptionUpdate(BaseModel):
    ...
    subject_type: str
    subject_id: str
    #: Место внутри сущности; пусто — общее обсуждение. Ветка — это место, а не сущность
    #: целиком: ушедший из обсуждения строки I5 не переставал следить за вкладкой «Сбыт».
    anchor: str = ""
    muted: bool = True


class ThreadUnsubscribeRequest(BaseModel):
    """Отписка по ссылке из письма. Ветку несёт **токен**, а не тело запроса: иначе
    ссылку можно было бы переписать и отписать человека от чужого разговора."""

    token: str
    #: ``False`` — «верните письма»: отписка, из которой нет дороги назад, это ловушка.
    muted: bool = True
