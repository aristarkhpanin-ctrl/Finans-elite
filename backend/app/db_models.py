...
class Payment(Base):
    ...
    **RLS-политики у этой таблицы нет — единственной среди таблиц с
    ``organization_id``, — и это решение, а не упущение** (F2). Вебхук провайдера
    приходит снаружи и знает только **свой** идентификатор платежа: организацию он
    не называет, она **выводится из найденной строки**. Под `FORCE ROW LEVEL SECURITY`
    такое чтение вернуло бы пустоту на PostgreSQL и молча потеряло бы оплату, а
    политика с лазейкой («или арендатор не выставлен») дала бы контур, в котором RLS
    не действует, — ровно то, что служебный контур отказался заводить в B1.
