...
def clear_tenant(db: Session) -> None:
    """Снять арендатора: до следующего :func:`set_tenant` запрос не увидит ни строки.

    Нужна служебному контуру (B1): сотрудник платформы обходит организации по очереди,
    и оставленный от предыдущей арендатор — это открытая дверь в чужие данные, которую
    никто не заметит. **Обхода RLS у платформы нет**: оператор входит в организацию
    через ту же дверь, что и её участники, и выходит из неё явно.
    """
    set_tenant(db, "")
