...
def build_saas() -> ProjectModel:
    ...
    return ProjectModel(
        ...
        operating_plan=OperatingPlan(
            # Хостинг задан ценой **на абонента**, а не суммой: тогда он следует за базой
            # сам, и второго ряда, который надо помнить и править вместе с притоком, нет.
            materials=[Material(id="m_host", name="Хостинг на абонента", unit="абон.",
                                unit_price=d(60))],
            products=[Product(id="p_sub", name="Подписка, мес.",
                              bom=[BomLine(material_id="m_host", qty_per_unit=d(1))])],
            # Объём выводится из базы: 150 абонентов на старте, 70 приходят, 3% уходят
            # каждый месяц. База стремится к 70/0,03 ≈ 2330 — и это главное, что шаблон
            # показывает: с оттоком рост не бесконечен, у него есть потолок, и достигается
            # он не приростом, а тем, что приток сравнялся с выбытием.
            sales=[SalesLine(product_id="p_sub", price=[d(2400)] * n,
                             payment=PaymentTerms(prepayment_share=d(1)),
                             subscription=Subscription(starting_base=d(150),
                                                       new_per_month=[d(70)] * n,
                                                       churn_monthly=d("0.03")))],
            staff=[
...
INDUSTRY_TEMPLATES: dict[str, Template] = {
    t.id: t for t in [
        ...
        Template(
            ...
            shows="Абонентская база с притоком и оттоком, предоплата (деньги раньше "
                  "выручки), НМА как класс актива, хостинг ценой на абонента.",
            assumptions=[
                NOT_A_BENCHMARK,
                "Приток 70 абонентов в месяц постоянен — самое смелое допущение шаблона: "
                "реальная воронка так себя не ведёт, она зависит от расходов на "
                "привлечение и от сезона.",
                "Отток 3% в месяц задан одним числом на весь горизонт. В жизни он выше в "
                "первые месяцы жизни абонента и ниже потом; одна ставка это усредняет и "
                "занижает потери на старте.",
                "Потолок базы (≈2330 абонентов) — следствие этих двух чисел, а не план: "
                "приток делённый на отток. Меняя любое из них, вы меняете потолок.",
                "Стоимость привлечения задана суммой, а не ценой за абонента; связь "
                "«потратили — пришли» в модели не выражена, и приток на неё не реагирует.",
                "Горизонт 4 года выбран не случайно: окупаемость наступает к третьему "
                "году, и это правда о подписке, а не ошибка расчёта.",
            ],
            build=build_saas),
        Template(
