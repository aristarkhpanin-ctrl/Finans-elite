...
- **Годовые показатели** (рентабельность, оборачиваемость активов) приводятся к году по типу
  периода (квартал → ×4): сопоставимость год/квартал. Показатели «в днях» считаются по
  длине периода и приведения не требуют.
...
def _analyze(model: AuditSubjectModel) -> AuditResult:
    ...
    # ── Коэффициенты ──────────────────────────────────────────────────────────
    # Приведение к году: квартальный поток ×4 (сопоставимость год/квартал).
    days = [_DAYS.get(p.kind, _YEAR) for p in model.periods]
    yr = [_YEAR / d for d in days]
