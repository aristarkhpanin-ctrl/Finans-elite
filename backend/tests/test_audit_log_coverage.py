...
NOT_LOGGED: dict[str, str] = {
    ...
    # Письма об обсуждениях (OPEN-DECISIONS §5) — личная настройка человека, а не
    # содержимое организации: у строки нет даже ``organization_id``. Записать «Иван
    # отписался от обсуждения» в журнал компании значило бы рассказать коллегам о том,
    # что он не хочет читать, — и это ровно та слежка, от которой журнал должен быть
    # свободен. Исход самих писем пишется в журнал (`comment.reply_mail`).
    "POST /api/v1/comments/subscription": "личная настройка писем, не данные организации",
    "POST /api/v1/comments/unsubscribe": "личная настройка писем, не данные организации",
    # Организация удаляется вместе со своим журналом: он её собственность, и писать в
    # него «вас больше нет» некуда и незачем. Событие уходит в **служебный журнал
    # платформы** (`log_staff_action`) — уход клиента она обязана видеть. Признать это
    # достаточным для общего перечня нельзя: иначе любой маршрут закрывал бы требование
    # записью в журнал, которого клиент не видит.
    "DELETE /api/v1/organizations/{org_id}": "журнал уходит с организацией; событие — в служебный",
