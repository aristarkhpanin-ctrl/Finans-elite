# Архитектура SaaS (финансовое моделирование)

> **Назначение документа.** Детальный архитектурный проект SaaS-платформы финансового
> моделирования: web-интерфейс, подписочная модель оплаты и запас на расширение.
>
> **Статус:** проектирование архитектуры — «карта», по которой идёт реализация.
...
| **Объём** | Полный функционал сразу | Модель данных и расчётное ядро проектируются под весь функционал (включая холдинг, what-if, Монте-Карло, актуализацию, язык формул) с самого начала |
...
5. [Доменная модель данных](#5-доменная-модель-данных)
...
### Цели
- **Полный функционал** финансового моделирования предприятия и инвестпроектов.
...
| **Worker** | Celery | Тяжёлые расчёты (Монте-Карло, актуализация холдинга), экспорт |
...
| **Object Storage** | S3-совместимое (Yandex/VK) | Файлы экспорта и вложения |

> **Почему модульный монолит, а не микросервисы сразу.** При таком объёме доменов
...
Модель проекта — структурированное дерево в Postgres, а не файл.
...
> разделов) и надёжность реляционной части (права, биллинг,
> результаты).
...
Состав повторяет разделы финансовой модели. Каждый блок — поддокумент
...
- **Golden master:** набор эталонных проектов с зафиксированными ожидаемыми числами;
  любые расхождения ловятся регрессией.
...
Встроенный язык формул — ~40 функций над
...
> Это отдельная, тщательно протестированная подсистема: от неё зависит и гибкость
> модели, и правильность чисел, которые она даёт.
...
| **REST API** | Программный доступ к модели и результатам |
...
Цель — **полный функционал**. Сборка идёт фазами (нельзя выпустить всё одним коммитом);
порядок выбран так, чтобы каждая фаза давала работающий вертикальный срез.
...
| **6. Интеграции** | Публичный API, расширенная коллаборация | Платформа открыта наружу |

> Golden-master тесты `calc_core` ведём начиная с Фазы 1 и наращиваем с каждым новым
> разделом — это страхует числа от незаметного дрейфа.
...
|---|---|---|
| **Правильность цифр** (методика расчёта) | Высокое | Детальная спецификация формул + golden-master регрессии |
...
| Модель проекта | Структурированное описание входных данных расчёта |
...
*Документ описывает целевую архитектуру; порядок реализации — в дорожной карте (§19).*
