...
> валютная позиция `B6`, долг `B26`, капитал `B27`, нераспределённая прибыль `B32`) задаётся
> стартовый оборотный капитал: **дебиторка** `B2`, **кредиторка** `B23` (0.9.6) и **запасы**
> сырья `B3`/ГП `B5` (0.9.9). Все входят в проверку сходимости стартового баланса (актив =
> пассив). Монетарные дебиторка/кредиторка «разворачиваются» в денежный поток в первом месяце
> (инкассация `C1`/оплата `C2`); запасы для действующего предприятия — **поддерживаемый
> уровень** (постоянны в `B3`/`B5`, потребляются и пополняются, денежного потока не создают).
> Авансы и детальная структура капитала, оборачиваемость стартовых запасов — к сверке по эталону.
