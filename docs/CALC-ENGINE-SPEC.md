...
3. **Курсовая разница `I25`:** перечень переоцениваемых статей и знак.
   ...
   **валютных займов** (`Loan.foreign`, `0.8.1`), **экспортной выручки**
   (`SalesLine.foreign`, `0.8.2`) и **валютных издержек-услуг** (`FixedCostLine.foreign`,
   `0.8.3`). Переоцениваются по `fx_rate[t]` → `I25`: активы/дебиторка — доход при росте
   курса; долг/кредиторка — убыток. Стоимость в `B6`/`B26`/`B2`/`B23`; баланс сходится,
   курсовая разница в налоговой базе. Остаётся валютное сырьё (взаимодействие с запасами)
   и реализационный учёт налога на курсовую разницу — к сверке по эталону.
