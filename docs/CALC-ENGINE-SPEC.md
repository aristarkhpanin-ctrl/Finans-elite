...
- **Валютный материал** (`DirectCostLine.foreign`, реализовано в 0.9.1): закупка/запас
  ...
  **монетарная**, переоценивается по `fx[t]` → курсовая разница `I25` (рост курса →
  убыток), оплата — по курсу периода. НДС на импорт в v0 не начисляется (к сверке по эталону).
...
3. **Курсовая разница `I25`:** перечень переоцениваемых статей и знак.
   ...
   **валютных займов** (`Loan.foreign`, `0.8.1`), **экспортной выручки**
   ...
   закупки**, без переоценки), переоценивается лишь **кредиторка** за материал; НДС на
   импорт в v0 не начисляется. Остаётся импортный НДС и реализационный учёт налога на
   курсовую разницу — к сверке по эталону.
