...
- **Оценка бизнеса:** 5 методов — ликвидационная стоимость, предполагаемая продажа,
  ...
  **DDM** (дисконтирование дивидендов) и мультипликаторы.
  - ✅ реализовано в 0.9.0 (`reports/valuation.py`, поле `valuation` в результате): **чистые
    активы** = собственный капитал на конец горизонта (B33); **модель Гордона** на
    нормализованном годовом свободном денежном потоке (последние ≤12 мес. `C13+C20`,
    приведённые к году), `g` — `settings.terminal_growth_rate`, `r` — ставка
    дисконтирования; `gordon_value = None` при `r ≤ g`. Остальные методы (ликвидационная,
    рыночные чистые активы, DDM, мультипликаторы) и точная база CF/период нормализации —
    к сверке по эталону, следующая фаза.
