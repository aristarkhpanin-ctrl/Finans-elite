...
### Валюты
- Проект ведёт **две валюты** (основная и вторая). Курс `FX[t]` — временной ряд.
...
- **Запасы готовой продукции** `B5[t]` = накопленный (произведённый − отгруженный)
  объём × себестоимость единицы; растут при производстве впрок, падают при отгрузке.
...
> (выручка). Их динамика обеспечивает корректную стыковку accrual/cash.
...
- **НДС к уплате в бюджет** входит в строку **Налоги** `C12`:
  `VAT_budget[t] = VATout_collected[t] − VATin_offset[t]`
  (по методу «по отгрузке» или «по оплате» — определяет момент признания).
- **Возврат/переплата НДС:** при включённом режиме возврата — отражается в `C12` со
  **знаком минус**; невозмещённый входной НДС копится в `B7` (предоплаченные расходы),
  при методе «по оплате» соответствующая часть — в `B23` (счета к оплате).

### Налог на прибыль
- База — **налогооблагаемая прибыль** `I26` (см. §12), ставка `tax_profit`.
- **Льготы:** доля прибыли, освобождаемая от налога, вычитается из базы.
- **Перенос убытков:** `I22` (убытки предыдущих периодов) уменьшают базу.
...
| **I26** | **Налогооблагаемая прибыль** | **`I23 + I24 + I25 − I22 − льготы`** |
| I27 | Налог на прибыль | `max(0, I26) · tax_profit` |
| **I28** | **Чистая прибыль** | **`I23 + I25 − I27`** |

> **Важно (нюанс паритета).** `I24` **добавляется** к налогооблагаемой базе `I26`
> (это «непринятые» к вычету расходы), но **не вычитается** повторно при расчёте чистой
> прибыли `I28` (она уже «выше» на эту величину, т.к. `I23` их не включал). Фактическая
> денежная выплата по `I24`-статьям проходит в Кэш-фло и уменьшает нераспределённую
> прибыль/наличность. Точную проводку `I24` обязательно сверить по эталону (см. §22).
...
Считаются на каждый период; балансовые величины берутся **средними за период**, из ОПУ
— итоговые. Для приведения к году при шаге < года применяется множитель (12/4/2).
`No` — число обыкновенных акций. Формулы по стандартным определениям (нотация строк §1):
