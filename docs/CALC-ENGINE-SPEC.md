...
3. **Курсовая разница `I25`:** перечень переоцениваемых статей и знак.
   ...
   **валютных займов** (`Loan.foreign`, `0.8.1`) и **экспортной выручки**
   (`SalesLine.foreign`, `0.8.2` — выручка по курсу отгрузки, валютная дебиторка
   переоценивается при оплате). Все переоцениваются по `fx_rate[t]` → `I25` (актив/доход
   при росте курса; долг — убыток); стоимость в `B6`/`B26`/`B2`; баланс сходится,
   курсовая разница в налоговой базе. Остальные валютные статьи (издержки/кредиторка) и
   реализационный учёт налога на курсовую разницу — следующие шаги Фазы B (к сверке).
