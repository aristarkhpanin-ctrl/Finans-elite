...
> стартовый оборотный капитал: **дебиторка** `B2` и **кредиторка** `B23` (реализовано в
> 0.9.6). Они входят в проверку сходимости стартового баланса (актив = пассив) и
> «разворачиваются» в денежный поток в первом месяце: дебиторка инкассируется (приток `C1`),
> кредиторка оплачивается (отток через `C2`). Остальные статьи стартового баланса (авансы,
> запасы, детальная структура капитала) — к сверке по эталону.
