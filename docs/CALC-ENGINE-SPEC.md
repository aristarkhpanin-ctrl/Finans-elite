...
3. **Курсовая разница `I25`:** перечень переоцениваемых статей и знак.
   ✅ Механизм реализован: монетарная позиция во 2-й валюте (`foreign_monetary`,
   `0.8.0`) и **валютные займы** (`Loan.foreign`, `0.8.1`) переоцениваются по `fx_rate[t]`
   → `I25` (актив: доход при росте курса; долг: убыток при росте курса); стоимость в
   `B6`/`B26`; баланс сходится, курсовая разница входит в налоговую базу. Остальные
   валютные статьи (выручка/издержки, дебиторка/кредиторка) и реализационный учёт налога
   на курсовую разницу — следующие шаги Фазы B (к сверке по эталону).
