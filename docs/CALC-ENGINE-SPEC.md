...
- **Оценка бизнеса:** 5 методов — ликвидационная стоимость, предполагаемая продажа,
  ...
  **DDM** (дисконтирование дивидендов) и мультипликаторы.
  - ✅ реализовано в `reports/valuation.py` (поле `valuation` в результате): **чистые
    ...
    чистая прибыль` (`I28`, `settings.valuation_earnings_multiple`, 0.9.5). `g` —
    `terminal_growth_rate`, `r` — ставка дисконтирования; капитализации = `None` при
    `r ≤ g`, мультипликатор = `None` при нулевом множителе. Ликвидационная стоимость,
    рыночные чистые активы и точные конвенции нормализации — к сверке по эталону.
